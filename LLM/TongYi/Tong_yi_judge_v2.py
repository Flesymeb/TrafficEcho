...
class TextAnalyzer:
    ...
    def __init__(self, api_key, base_url, input_filename):
        ...
        # 将输入文件路径设置为 'input/' 文件夹下的文件
        self.input_filename = f"input/{input_filename}"
        # 输出文件的名称（json格式）
        self.output_filename = f"{input_filename.split('.')[0]}_judge_result.json"
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "请判断文本是否包含有效的交通情感评价，仅返回有效或无效,不需要其他的理由与判断原因。",
                    },
    ...
    def append_filtered_comments_to_json_file(self, valid_lines):
        output_dir = "output"
        try:
            # 读取输入文件内容
            ...
            # 创建一个列表来保存有效评论数据
            filtered_comments = []
            for i, line_num in enumerate(valid_lines, start=1):
                # 获取对应行号的评论文本
                comment_text = data[line_num - 1].strip()  # 保留完整评论
                filtered_comments.append({"微博正文": comment_text})  # 仅包含评论内容

            # 将有效评论保存到 JSON 文件
            output_file_path = f"{output_dir}/{self.output_filename}"
            with open(output_file_path, "w", encoding="utf-8") as outfile:
                json.dump(filtered_comments, outfile, ensure_ascii=False, indent=4)
            print(f"有效评论已保存到: {output_file_path}")
    ...
    def process_data(self):
        ...
        if valid_lines:
            self.append_filtered_comments_to_json_file(
                valid_lines
            )  # 将有效评论写入 JSON 文件
        else:
...
if __name__ == "__main__":
    analyzer = TextAnalyzer(
        ...
        input_filename="公交 晚点.txt",  # 输入的是txt文件，位于input文件夹
    )
