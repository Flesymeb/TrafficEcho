...
class SearchSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for keyword in self.keyword_list:
            ...
            url = f"{base_url}&scope=ori&suball=1&timescope=custom:{start_str}:{end_str}"

            # 打印构造的URL以调试
            self.logger.debug(f"构造请求 URL: {url}")
    ...
    def _get_headers(self):
        """返回默认请求头，确保 Cookie 和 User-Agent 配置正确"""
        headers = self.settings.get('DEFAULT_REQUEST_HEADERS', {})
        if 'Cookie' not in headers or not headers['Cookie']:
            raise ValueError("Cookie 未正确设置，请检查 settings.py 中的 DEFAULT_REQUEST_HEADERS 配置")
        return headers
    ...
    def get_ip(self, bid):
        ...
        if response.status_code != 200:
            self.logger.warning(f"获取IP失败，状态码: {response.status_code}")
            return ""
        try:
            data = response.json()
        except requests.exceptions.JSONDecodeError:
            self.logger.warning("无法解析IP JSON响应")
            return ""
        ip_str = data.get("region_name", "")
        if ip_str:
            ip_str = ip_str.split()[-1]
    ...
    def parse(self, response):
        """解析搜索结果页面"""
        self.logger.debug(f"当前请求 URL: {response.url}")
        is_empty = response.xpath('//div[@class="card card-no-result s-pt20b40"]')
        if is_empty:
            self.logger.info("当前页面无搜索结果")
            return
        for weibo in self.parse_weibo(response):
            yield weibo
