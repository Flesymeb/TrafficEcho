...
for subdir, dirs, files in os.walk(main_dir):
    for file in files:
        if file.endswith('.csv'):
            ...
            df.drop_duplicates(subset='id', keep='first', inplace=True)  # 删掉重复内容(基于ID，考虑转发)
            df.dropna(subset=['ip'], inplace=True)  # 删除IP为空的行
            df = df[df['微博正文'].apply(lambda x: len(str(x)) <= lim)]  # 去掉微博正文长度大于某个极限的评论
            df = df[df['微博正文'].apply(lambda x: not str(x).startswith("【"))] # 去掉微博正文以"【"开头的评论
