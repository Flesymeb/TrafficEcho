...
def construct_messages(comment, categories):
    ...
    system_message = (
        f"你是一个专业的交通评价分析师，你的任务是对用户提供的微博内容进行主观评价维度上的分类。返回分类结果，以及分类的理由。评价维度分类指标包括:{category_str}。\n"
        f"以下是对分类指标的一些解释:\n"
...
def parse_output(output, categories):
    ...
    if "类别:" in output and "理由:" in output:
        try:
            label_part, reasoning = output.split("理由:", 1)
            label_part = label_part.replace("类别:", "").strip()
            labels = [cat.strip() for cat in label_part.split(",") if cat in categories]
        except ValueError:
...
def process_comments(comments, categories, client, log_file):
    """
    处理所有评论并获取分类结果。
    """
    results = []
    with open(log_file, 'a', encoding='utf-8') as log:
        for comment_data in comments:
            ...
            # 解析输出
            predicted_label, reasoning = parse_output(api_output, categories)

            # 记录完整输出到日志
            log.write(f"微博正文: {comment}\n")
            log.write(f"预测标签: {predicted_label}\n")
            log.write(f"理由: {reasoning}\n")
...
def run_classification(input_file, output_file, log_file, categories):
    """
    执行分类任务。
    """
    client = OpenAI(api_key="sk-712b4d4feb1b4f2289f33334f0a7e57e",
    ...
    # 处理评论
    results = process_comments(comments, categories, client, log_file)
...
if __name__ == "__main__":
    ...
    base_output_path = "classification_evaluation_0306test"  # 基础输出路径
    log_reason = "class_eva_0306.log"
    ...
    # 创建日志文件
    log_file = "classification_evaluation_0306test/class_eva.log"
    with open(log_file, 'a', encoding='utf-8') as log:
    ...
    for run_idx in range(1, num_runs + 1):
        ...
        # 执行分类任务
        run_classification(input_file, output_file, log_reason, categories)
    ...
    # 保存平均值到文件
    with open("classification_evaluation_0306test/average_metrics.json", "w", encoding="utf-8") as f:
