...
# 配置通义千问API
API_KEY = "sk-712b4d4feb1b4f2289f33334f0a7e57e"  # 替换为您的实际API密钥
...
# 设置全局日志文件
log_file = "logs_nenore/negotiation_process.log"
os.makedirs("logs_nenore", exist_ok=True)
...
def discriminator_evaluate(original_content, generator_label, generator_reason, feedback=""):
    prompt = (
        f"## 你是一个专业的交通评价分析师，仅根据微博的内容，请评估生成器G关于该微博内容在主观评价维度上的分类是否合理，不要解释理由。\n"
        f"## 类别列表：{categories}\n"
        f"以下是对分类指标的一些解释:\n"
        f"经济性（Economic Efficiency）：涉及到花钱、票价、燃油费等关于出行的花费的评价、涉及到运营成本、投资回报率的就是经济性相关；\n"
        f"安全性（Safety）：涉及到交通事故、危险驾驶（超车加塞等）、交通运营安全、设施安全性、乘客安全、运行环境安全、应急响应、事故预防能力（比如查酒驾、交警检查、交通管制等，但只涉及到交警等不算）的，就是安全性相关\n"
        ...
        f"如果你认为分类合理，请返回“同意”；如果你认为分类不合理，请返回“不同意”，不要解释理由\n\n"
        f"## 微博内容：\n"
        f"{original_content}\n\n"
        f"## 生成器G的分类结果：\n"
...
def negotiate_label(weibo_content, max_rounds=3, tolerance=0.1):
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        try:
            # 判别器评估
            d_agreement, d_tokens = discriminator_evaluate(
        ...
        if prev_label == g_label:
            negotiation_log.append(f"稳定终止（{g_label}）")
            logger.warning("\n".join(negotiation_log))
            return g_label, g_reason, final_round
...
if __name__ == "__main__":
    ...
    output_file = "classified_data_test_0306nor.json"
